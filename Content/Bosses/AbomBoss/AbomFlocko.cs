                ...
                if (FargoSoulsUtil.HostCheck)
                {
                    if (Math.Abs(npc.Center.X - Projectile.Center.X) > 400)
                    {
                        for (int i = 0; i < 2; i++)
                        {
                            ...
                            speed.Normalize();
                            speed *= 8f;
                            Projectile.NewProjectile(npc.GetSource_FromThis(), Projectile.Center + speed * 4f, speed, ModContent.ProjectileType<Bosses.AbomBoss.AbomFrostShard>(), Projectile.damage, Projectile.knockBack, Projectile.owner);
                        }
                    }

...
{
    public class AbomBossOverride : GlobalNPC
    {
        // ========== 基础配置 ==========
        public override bool InstancePerEntity => true;
        private bool customAIInitialized = false;
        ...
        private bool playerInvulTriggered = false;
        ...
        public override bool AppliesToEntity(NPC npc, bool lateInstantiation)
        {
            ...
            {
                return npc.type == fargoMod.Find<ModNPC>("AbomBoss").Type;
            }
            return false;
        }
        // ========== 修改基础属性 ==========
        ...
        {
            if (!AppliesToEntity(npc, false)) return;
            npc.damage = 150; // 原版240
            npc.defense = 80;
            npc.lifeMax = 560000; // 原版680000
        ...
        {
            CooldownSlot = 1;
            return NPC.Distance(FargoSoulsUtil.ClosestPointInHitbox(target, NPC.Center)) < Player.defaultHeight && NPC.ai[0] != 0 && NPC.ai[0] != 10 && NPC.ai[0] != 18 && NPC.ai[0] != 23 && NPC.ai[0] != 24 && NPC.ai[0] != 28;
        ...
        public override void OnSpawn(NPC NPC,IEntitySource source)
        {
            ...
            NPC.aiStyle = -1;
            NPC.netUpdate = true;
            int[] rituals = [ModContent.ProjectileType<Content.Bosses.AbomBoss.AbomRitual>(), ModContent.ProjectileType<Content.Bosses.AbomBoss.AbomRitualMaso>(), ModContent.ProjectileType<Content.Bosses.AbomBoss.AbomRitualFTW>(), ModContent.ProjectileType<Content.Bosses.AbomBoss.AbomRitual2>()];
            ...
            Main.dayTime = false;
            Main.time = 0; // 午夜
            Main.bloodMoon = true; // 启用血月
            Main.eclipse = false; // 确保日食关闭
        ...
        {
            if (!AppliesToEntity(npc, false)) return true;
            // 初始化魔改AI
        ...
        private void InitializeCustomAI(NPC npc)
        {
            ...
            npc.localAI[0] = Main.rand.Next(3); // 随机起始强攻击
            npc.localAI[1] = Main.rand.Next(2); // 随机起始超级攻击
            npc.localAI[2] = 0; // 通用局部状态
            npc.localAI[3] = 0; // 阶段：0=准备，1=P1，2=P2
        ...
        private void ExecutePreAI(NPC npc)
        {
            ...
            if (FargoSoulsUtil.HostCheck)
            {
                // 生成仪式弹幕逻辑...
                // 保持与原代码一致
                if (WorldSavingSystem.EternityMode && npc.localAI[3] == 2 && FargoSoulsUtil.ProjectileExists(ritualProj, ModContent.ProjectileType<Content.Bosses.AbomBoss.AbomRitual>()) == null)
                    ritualProj = Projectile.NewProjectile(npc.GetSource_FromThis(), npc.Center, Vector2.Zero, ModContent.ProjectileType<Content.Bosses.AbomBoss.AbomRitual>(), FargoSoulsUtil.ScaledProjectileDamage(npc.defDamage), 0f, Main.myPlayer, 0f, npc.whoAmI);

                if (WorldSavingSystem.MasochistModeReal && npc.localAI[3] > 0 && FargoSoulsUtil.ProjectileExists(ritualProjMaso, ModContent.ProjectileType<Content.Bosses.AbomBoss.AbomRitualMaso>()) == null)
                    ritualProjMaso = Projectile.NewProjectile(npc.GetSource_FromThis(), npc.Center, Vector2.Zero, ModContent.ProjectileType<Content.Bosses.AbomBoss.AbomRitualMaso>(), FargoSoulsUtil.ScaledProjectileDamage(npc.defDamage), 0f, Main.myPlayer, 0f, npc.whoAmI);

                if (Main.getGoodWorld && npc.localAI[3] == 2 && FargoSoulsUtil.ProjectileExists(ritualProjFTW, ModContent.ProjectileType<Content.Bosses.AbomBoss.AbomRitualFTW>()) == null)
                    ritualProjFTW = Projectile.NewProjectile(npc.GetSource_FromThis(), npc.Center, Vector2.Zero, ModContent.ProjectileType<Content.Bosses.AbomBoss.AbomRitualFTW>(), FargoSoulsUtil.ScaledProjectileDamage(npc.defDamage), 0f, Main.myPlayer, 0f, npc.whoAmI);

                if (FargoSoulsUtil.ProjectileExists(ringProj, ModContent.ProjectileType<Content.Bosses.AbomBoss.AbomRitual2>()) == null)
                    ringProj = Projectile.NewProjectile(npc.GetSource_FromThis(), npc.Center, Vector2.Zero, ModContent.ProjectileType<Content.Bosses.AbomBoss.AbomRitual2>(), 0, 0f, Main.myPlayer, 0f, npc.whoAmI);

                if (FargoSoulsUtil.ProjectileExists(spriteProj, ModContent.ProjectileType<Content.Bosses.AbomBoss.AbomBossProjectile>()) == null)
                {
                    if (Main.netMode == NetmodeID.SinglePlayer)
                    {
                        ...
                        if (number >= 0)
                        {
                            if (Main.netMode == NetmodeID.SinglePlayer)
                            {
                                ...
                                projectile.SetDefaults(ModContent.ProjectileType<Content.Bosses.AbomBoss.AbomBossProjectile>());
                    ...
                    {
                        spriteProj = Projectile.NewProjectile(npc.GetSource_FromThis(), npc.Center, Vector2.Zero, ModContent.ProjectileType<Content.Bosses.AbomBoss.AbomBossProjectile>(), 0, 0f, Main.myPlayer, 0, npc.whoAmI);
                    }
                }
            }
            ...
            if (Main.LocalPlayer.active && npc.Distance(Main.LocalPlayer.Center) < 3000f)
            {
                if (WorldSavingSystem.EternityMode)
                {
                    Main.LocalPlayer.AddBuff(
                        ModContent.BuffType<FargowiltasSouls.Content.Buffs.Boss.AbomPresenceBuff>(), 2);
                }
            ...
            if (npc.localAI[3] == 2)
            {
                ...
                /*
                if (!SkyManager.Instance["FargosPhantasmMode:AbomSky2"].IsActive())
                {
                    SkyManager.Instance.Activate("FargosPhantasmMode:AbomSky2");
                }
                */
            }
        }
            ...
            switch ((int)npc.ai[0])
            {
                ...
                case -1: //phase 2 transition
                    ...
                    else if (NPC.ai[1] == 120)
                    {
                        ...
                        if (FargoSoulsUtil.HostCheck && FargoSoulsUtil.ProjectileExists(ritualProj, ModContent.ProjectileType<Content.Bosses.AbomBoss.AbomRitual>()) == null)
                        {
                            ritualProj = Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, Vector2.Zero, ModContent.ProjectileType<Content.Bosses.AbomBoss.AbomRitual>(), FargoSoulsUtil.ScaledProjectileDamage(NPC.defDamage), 0f, Main.myPlayer, 0f, NPC.whoAmI);
                        }
                        SoundEngine.PlaySound(SoundID.Roar, NPC.Center);
                ...
                case 0: //track player, throw scythes 
                        ...
                        if (NPC.ai[3] == 0)
                        {
                            ...
                            if (WorldSavingSystem.MasochistModeReal) //phase 2 saucers
                            {
                                ...
                                for (int i = 0; i < max; i++)
                                {
                                    ...
                                    FargoSoulsUtil.NewNPCEasy(NPC.GetSource_FromAI(), NPC.Center, ModContent.NPCType<Content.Bosses.AbomBoss.AbomSaucer>(), 0, NPC.whoAmI, 0, ai2);
                                }
                            }
                        }
                    }

                    if (NPC.ai[1] == 120 - Content.Bosses.AbomBoss.AbomStyxGazer.TelegraphTime)
                    {
                        if (NPC.ai[2] < (WorldSavingSystem.MasochistModeReal ? 9 : 6) && FargoSoulsUtil.HostCheck)
                        {
                            ...
                            float rotation = MathHelper.Pi * 1f * Content.Bosses.AbomBoss.AbomStyxGazer.Direction;
                            Content.Bosses.AbomBoss.AbomStyxGazer.Direction *= -1;
                            Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, NPC.DirectionTo(player.Center).RotatedBy(rotation * 0.6f),
                                ModContent.ProjectileType<Content.Bosses.AbomBoss.AbomStyxGazer>(), FargoSoulsUtil.ScaledProjectileDamage(NPC.defDamage), 0f, Main.myPlayer, NPC.whoAmI, rotation / 60 * 2);
                        }
                    }
                    ...
                    if (NPC.ai[1] > 120)
                    {
                        ...
                        else if (FargoSoulsUtil.HostCheck)
                        {
                            ...
                            Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, NPC.SafeDirectionTo(player.Center) * 30f, ModContent.ProjectileType<Content.Bosses.AbomBoss.AbomScytheSplit>(), FargoSoulsUtil.ScaledProjectileDamage(NPC.defDamage), 0f, Main.myPlayer, ai0, ai1);
                        }
                    }
                    /*else if (NPC.ai[1] == 90)
                    {
                            ...
                            else
                            {
                                    ...
                                    if (FargoSoulsUtil.HostCheck && NPC.HasPlayerTarget)
                                    {
                                        for (int i = 0; i < max; i++)
                                        {
                                            Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center,
                                                ...
                                                ModContent.ProjectileType<Content.Bosses.AbomBoss.AbomScytheFlaming>(),
                                ...
                                else // P1阶段 - 使用改良版但保持稳定性
                                {
                                    ...
                                    if (FargoSoulsUtil.HostCheck && NPC.HasPlayerTarget)
                                    {
                                        for (int i = 0; i < max; i++)
                                        {
                                            Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, NPC.SafeDirectionTo(player.Center).RotatedBy(MathHelper.TwoPi / max * (i + offset)) * speed, ModContent.ProjectileType<Content.Bosses.AbomBoss.AbomScytheFlaming>(), FargoSoulsUtil.ScaledProjectileDamage(NPC.defDamage), 0f, Main.myPlayer, baseDelay, baseDelay + extendedDelay, ai2: NPC.target);
                                        }
                                    }
                                }
                    ...
                    if (++NPC.ai[1] > windup)
                    {
                        ...
                        else
                        {
                            ...
                            if (FargoSoulsUtil.HostCheck)
                            {
                                ...
                                Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, Vector2.Normalize(NPC.velocity).RotatedBy(-rotation / 2),
                                    ModContent.ProjectileType<Content.Bosses.AbomBoss.AbomStyxGazerDash>(), FargoSoulsUtil.ScaledProjectileDamage(NPC.defDamage), 0f, Main.myPlayer, NPC.whoAmI, rotation / timeleft * 2, timeleft);
                            }
                ...
                case 3: //while dashing (p2 makes side scythes)
                    ...
                    if (++NPC.ai[3] > 5)
                    {
                        ...
                        if (FargoSoulsUtil.HostCheck)
                        {
                            Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, Vector2.Normalize(NPC.velocity), ModContent.ProjectileType<Content.Bosses.AbomBoss.AbomPhoenix>(), FargoSoulsUtil.ScaledProjectileDamage(NPC.defDamage), 0, Main.myPlayer);
                            if (NPC.localAI[3] > 1)
                            {
                                Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, Vector2.Normalize(NPC.velocity).RotatedBy(Math.PI / 2), ModContent.ProjectileType<Content.Bosses.AbomBoss.AbomPhoenix>(), FargoSoulsUtil.ScaledProjectileDamage(NPC.defDamage), 0, Main.myPlayer);
                                Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, Vector2.Normalize(NPC.velocity).RotatedBy(-Math.PI / 2), ModContent.ProjectileType<Content.Bosses.AbomBoss.AbomPhoenix>(), FargoSoulsUtil.ScaledProjectileDamage(NPC.defDamage), 0, Main.myPlayer);
                                if (Main.zenithWorld)
                                {
                                    for (float i = -1.5f ; i <= 1.5f;i += 3f)
                                    {
                                        Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, i * Vector2.Normalize(NPC.velocity).RotatedBy(Math.PI / 2), ModContent.ProjectileType<Content.Bosses.AbomBoss.AbomPhoenix>(), FargoSoulsUtil.ScaledProjectileDamage(NPC.defDamage), 0, Main.myPlayer);
                                    }
                                }
                            }
                        }
                        ...
                        else
                        {
                            if (FargoSoulsUtil.HostCheck)
                            {
                                ...
                                for (int i = 0; i < 4; i++)
                                {
                                    ...
                                    Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, straightSpeed, ModContent.ProjectileType<Content.Bosses.AbomBoss.AbomSickleSplit1>(), FargoSoulsUtil.ScaledProjectileDamage(NPC.defDamage), 0f, Main.myPlayer, NPC.whoAmI);

                                    Vector2 diagonalSpeed = new Vector2(baseSpeed, baseSpeed).RotatedBy(baseRot + Math.PI / 2 * i);
                                    Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, diagonalSpeed, ModContent.ProjectileType<Content.Bosses.AbomBoss.AbomSickleSplit1>(), FargoSoulsUtil.ScaledProjectileDamage(NPC.defDamage), 0f, Main.myPlayer, NPC.whoAmI);
                    ...
                    if (NPC.ai[2] == 0)
                    {
                        ...
                        if (FargoSoulsUtil.HostCheck)
                        {
                            for (int i = -3; i <= 3; i++) //make flockos
                            {
                                ...
                                Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, overheadSpeed, ModContent.ProjectileType<Content.Bosses.AbomBoss.AbomFlocko>(), FargoSoulsUtil.ScaledProjectileDamage(NPC.defDamage), 0f, Main.myPlayer, NPC.whoAmI, 360 / 3 * i);
                            }

                            //prepare ice waves

                            float offset = 420;
                            Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, Main.rand.NextVector2CircularEdge(20, 20), ModContent.ProjectileType<Content.Bosses.AbomBoss.AbomFlocko3>(), FargoSoulsUtil.ScaledProjectileDamage(NPC.defDamage), 0f, Main.myPlayer, NPC.whoAmI, offset);
                            Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, Main.rand.NextVector2CircularEdge(20, 20), ModContent.ProjectileType<Content.Bosses.AbomBoss.AbomFlocko3>(), FargoSoulsUtil.ScaledProjectileDamage(NPC.defDamage), 0f, Main.myPlayer, NPC.whoAmI, -offset);
                            if (NPC.localAI[3] <= 1)
                            {
                                Vector2 speed = new(Main.rand.NextFloat(40f), Main.rand.NextFloat(-20f, 20f));
                                Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, speed, ModContent.ProjectileType<Content.Bosses.AbomBoss.AbomFlocko2>(), FargoSoulsUtil.ScaledProjectileDamage(NPC.defDamage), 0f, Main.myPlayer, NPC.target, 0, NPC.localAI[3]);
                                Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, speed, ModContent.ProjectileType<Content.Bosses.AbomBoss.AbomFlocko2>(), FargoSoulsUtil.ScaledProjectileDamage(NPC.defDamage), 0f, Main.myPlayer, NPC.target, 180, NPC.localAI[3]);
                            }
                            else
                            {
                                Vector2 speed = new(Main.rand.NextFloat(40f), Main.rand.NextFloat(-20f, 20f));
                    ...
                    if (NPC.localAI[3] == 2 && NPC.ai[1] % (Main.zenithWorld ? 30 : 60) == 0 && NPC.ai[1] >= 60)
                    {
                        for (int i = -2; i <= 2; i++)
                        {
                            for (int j = -1; j <= 1; j += 2)
                            {
                                ...
                                Projectile.NewProjectile(NPC.GetSource_FromThis(), desiredPosition, direction.RotatedBy(i * MathHelper.Pi / 10) * 8, ModContent.ProjectileType<Content.Bosses.AbomBoss.AbomFrostWave>(), NPC.damage / 4, 0, NPC.target);
                            }
                            ...
                            if (FargoSoulsUtil.HostCheck)
                            {
                                if (NPC.localAI[3] > 1) //p2 shoots to either side of you
                                {
                                    ...
                                    for (int i = -3; i <= 3; i += 2)
                                    {
                                        ...
                                        Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, speed.RotatedBy(MathHelper.ToRadians(angleOffset * i)), ModContent.ProjectileType<Content.Bosses.AbomBoss.AbomLaser>(), FargoSoulsUtil.ScaledProjectileDamage(NPC.defDamage), 0f, Main.myPlayer);
                                    }
                                }
                                else //p1 shoots directly
                                {
                                    for (int i = 0; i < 2; i++)
                                    {
                                        ...
                                        Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, speed, ModContent.ProjectileType<Content.Bosses.AbomBoss.AbomLaser>(), FargoSoulsUtil.ScaledProjectileDamage(NPC.defDamage), 0f, Main.myPlayer);
                                        Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, speed.RotatedBy(Math.PI), ModContent.ProjectileType<Content.Bosses.AbomBoss.AbomLaser>(), FargoSoulsUtil.ScaledProjectileDamage(NPC.defDamage), 0f, Main.myPlayer);
                                    }
                                }
                            }
                        }
                        if (NPC.localAI[3] == 1)
                        {
                            if (++NPC.localAI[2] > 60)
                            {
                                ...
                                for (int i = 0; i < 7; i++)
                                {
                                    ...
                                    if (FargoSoulsUtil.HostCheck)
                                        Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, vel.RotatedBy(i * 2 * Math.PI / 7), ModContent.ProjectileType<Content.Bosses.AbomBoss.AbomRocket>(), FargoSoulsUtil.ScaledProjectileDamage(NPC.defDamage), 0f, Main.myPlayer, NPC.target, 20, ai2);
                                }
                            }
                        }
                        else
                        {
                            if (++NPC.localAI[2] % 15 == 0)
                            {
                                for (int i = 0; i < 4; i++)
                                {
                                    ...
                                    float ai2 = NPC.localAI[3] > 1 ? 0 : 1;
                                    if (FargoSoulsUtil.HostCheck)
                                        Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, vel.RotatedBy(i * 2 * Math.PI / 4), ModContent.ProjectileType<Content.Bosses.AbomBoss.AbomRocket>(), FargoSoulsUtil.ScaledProjectileDamage(NPC.defDamage), 0f, Main.myPlayer, NPC.target, 20, ai2);
                                }
                                for (int i = -1; i < 2;)
                                {
                ...
                case 9: //beginning of scythe rows and deathray rain
                    ...
                    if (++NPC.ai[1] == 1)
                    {
                        ...
                        if (FargoSoulsUtil.HostCheck)
                        {
                            Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, NPC.ai[3].ToRotationVector2(), ModContent.ProjectileType<Content.Bosses.AbomBoss.AbomDeathraySmall>(), 0, 0f, Main.myPlayer);
                            Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, -NPC.ai[3].ToRotationVector2(), ModContent.ProjectileType<Content.Bosses.AbomBoss.AbomDeathraySmall>(), 0, 0f, Main.myPlayer);
                        }
                    }
                    else if (NPC.ai[1] == 61)
                    {
                        ...
                        for (int j = -1; j <= 1; j += 2)
                        {
                            ...
                            for (int i = 1; i <= max + 2; i++)
                            {
                                ...
                                if (FargoSoulsUtil.HostCheck)
                                    Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, vel, ModContent.ProjectileType<Content.Bosses.AbomBoss.AbomScytheSpin>(), FargoSoulsUtil.ScaledProjectileDamage(NPC.defDamage, 4f * 3 / 8), 0f, Main.myPlayer, NPC.whoAmI, ai1);
                ...
                case 11: //dash and make deathrays
                    ...
                    if (++NPC.ai[3] > 5)
                    {
                        ...
                        if (FargoSoulsUtil.HostCheck)
                        {
                            ...
                            Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, vel1, ModContent.ProjectileType<Content.Bosses.AbomBoss.AbomDeathrayMark>(), FargoSoulsUtil.ScaledProjectileDamage(NPC.defDamage, 4f * 3 / 8), 0f, Main.myPlayer, timeLeft);
                            Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, vel2, ModContent.ProjectileType<Content.Bosses.AbomBoss.AbomDeathrayMark>(), FargoSoulsUtil.ScaledProjectileDamage(NPC.defDamage, 4f * 3 / 8), 0f, Main.myPlayer, timeLeft);
                        }
                    }
                ...
                case 18: //beginning of vertical dive
                    {
                        ...
                        if (NPC.ai[2] == 0 && NPC.ai[3] == 0) //target one side of arena
                        {
                            ...
                            if (FargoSoulsUtil.ProjectileExists(ritualProj, ModContent.ProjectileType<Content.Bosses.AbomBoss.AbomRitual>()) != null)
                            {
                ...
                case 19: //prepare to dash
                    ...
                    if (NPC.ai[1] == 0)
                    {
                        if (FargoSoulsUtil.HostCheck)
                        {
                            ...
                            Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, Vector2.UnitX * -horizontalModifier, ModContent.ProjectileType<Content.Bosses.AbomBoss.AbomSword>(), FargoSoulsUtil.ScaledProjectileDamage(NPC.defDamage, 4f * 3 / 8), 0f, Main.myPlayer, ai0, NPC.whoAmI);
                            if (WorldSavingSystem.MasochistModeReal)
                                Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, -Vector2.UnitX * -horizontalModifier, ModContent.ProjectileType<Content.Bosses.AbomBoss.AbomSword>(), FargoSoulsUtil.ScaledProjectileDamage(NPC.defDamage, 4f * 3 / 8), 0f, Main.myPlayer, ai0, NPC.whoAmI);
                        }
                    }
                                            ...
                                            for (float angleOffset = -spreadAngle; angleOffset <= spreadAngle; angleOffset += angleStep)
                                            {
                                                ...
                                                for (int i = 0; i < 4; i++)
                                                {
                                                    ...
                                                    Projectile.NewProjectile(
                                                        ...
                                                        spawnPos,
                                                        velocity,
                                                        ModContent.ProjectileType<Content.Projectiles.Masomode.BloodThornMissile>(),
                            ...
                            if (FargoSoulsUtil.HostCheck && Math.Abs(NPC.ai[1] - NPC.ai[2]) < 0.5f && NPC.ai[2] > 0)
                            {
                                ...
                                for (int i = 0; i < scytheCount; i++)
                                {
                                    ...
                                    Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, scytheVel,
                                        ModContent.ProjectileType<Content.Bosses.AbomBoss.ShadowFlamingScythe>(),
                                        FargoSoulsUtil.ScaledProjectileDamage(NPC.defDamage), 0f, Main.myPlayer);
                                }
                        ...
                        if (NPC.ai[1] >= 60 && NPC.ai[1] <= 240)
                        {
                            ...
                            if ((NPC.ai[1] - 60) % 20 == 0)
                            {
                                ...
                                if (FargoSoulsUtil.HostCheck)
                                {
                                    ...
                                    Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center,
                                        ...
                                        ModContent.ProjectileType<Content.Bosses.AbomBoss.AbomScytheSplit>(),
                                        FargoSoulsUtil.ScaledProjectileDamage(NPC.defDamage),
                                        0f, Main.myPlayer, ai0, ai1);
                            ...
                            if ((NPC.ai[1] - 60) % 20 == 0 && NPC.ai[1] > gazeSpawnTime + 20)
                            {
                                if (FargoSoulsUtil.HostCheck)
                                {
                                    // 冥河冥视的发射逻辑（模仿case0）
                                    float rotation = MathHelper.Pi * 1f * Content.Bosses.AbomBoss.AbomStyxGazer.Direction;
                                    Content.Bosses.AbomBoss.AbomStyxGazer.Direction *= -1; // 切换方向

                                    Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center,
                                        NPC.DirectionTo(player.Center).RotatedBy(rotation * 0.6f),
                                        ModContent.ProjectileType<Content.Bosses.AbomBoss.AbomStyxGazer>(),
                                        FargoSoulsUtil.ScaledProjectileDamage(NPC.defDamage),
                ...
                case 25: //尾杀双刃莱瓦汀12开始
                    ...
                    else if (NPC.ai[1] == 60 && FargoSoulsUtil.HostCheck)
                    {
                        ...
                        if (WorldSavingSystem.MasochistModeReal)
                            Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, -vel, ModContent.ProjectileType<Content.Bosses.AbomBoss.AbomSword>(), FargoSoulsUtil.ScaledProjectileDamage(NPC.defDamage, 4f * 3 / 8), 0f, Main.myPlayer, ai0, NPC.whoAmI, ai2: 1);
                    }
                    break;
                ...
                case 29: //prepare to dash
                    ...
                    if (NPC.ai[1] == 0)
                    {
                        if (FargoSoulsUtil.HostCheck)
                        {
                            ...
                            Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, Vector2.UnitY * -verticalModifier, ModContent.ProjectileType<Content.Bosses.AbomBoss.AbomSword>(), FargoSoulsUtil.ScaledProjectileDamage(NPC.defDamage, 4f * 3 / 8), 0f, Main.myPlayer, ai0, NPC.whoAmI);
                            if (WorldSavingSystem.MasochistModeReal)
                                Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, -Vector2.UnitY * -verticalModifier, ModContent.ProjectileType<Content.Bosses.AbomBoss.AbomSword>(), FargoSoulsUtil.ScaledProjectileDamage(NPC.defDamage, 4f * 3 / 8), 0f, Main.myPlayer, ai0, NPC.whoAmI);
                        }
                    }
        ...
        // ========== 辅助方法 ==========
        void GenerateBloodThorn(NPC NPC,float directionAngle, Vector2 Center, float radius)
        {
            // 将角度转换为弧度
        ...
        // 生成切线血弹的辅助方法
        void GenerateTangentialBloodThorns(NPC NPC,float directionAngle, Vector2 bossCenter, float spawnRadius, float tangentRadius)
        {
            // 将角度转换为弧度
            float angleRad = MathHelper.ToRadians(directionAngle);
                ...
                if (!player.active || player.dead || Vector2.Distance(npc.Center, player.Center) > 5000f)
                {
                    ...
                    if (npc.timeLeft == 1)
                    {
                        ...
                        if (FargoSoulsUtil.HostCheck)
                        {
                            ...
                            {
                                if (fargoMod.TryFind<ModNPC>("Abominationn", out ModNPC abomNPC) && !NPC.AnyNPCs(abomNPC.Type))
                                {
        ...
        private bool Phase2Check(NPC npc)
        {
            ...
            if (npc.life < npc.lifeMax * (WorldSavingSystem.EternityMode ? 0.66f : 0.50f) && Main.expertMode)
            {
                if (FargoSoulsUtil.HostCheck)
                {
                    npc.ai[0] = -1;
        ...
        private void MovementY(NPC NPC,float targetY, float speedModifier)
        {
        ...
        {
            if (!AppliesToEntity(npc, false)) return base.CheckDead(npc);
        ...
        {
            // 处理冰冻Buff清除
            if (npc.HasBuff<FargowiltasSouls.Content.Buffs.Souls.FrozenBuff>())
            {
                int frozen = npc.FindBuffIndex(ModContent.BuffType<FargowiltasSouls.Content.Buffs.Souls.FrozenBuff>());
                npc.DelBuff(frozen);
            }
                ...
                npc.HasPlayerTarget && !droppedSummon)
            {
                Item.NewItem(npc.GetSource_Loot(), player.Hitbox,
                    ModContent.ItemType<FargowiltasSouls.Content.Items.Summons.AbomsCurse>());
                droppedSummon = true;
            }
        }
